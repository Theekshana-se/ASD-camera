...
const CheckoutPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!checkoutForm.rentalDate) {
      errors.push("Rental date is required");
    } else {
      const requestedDate = new Date(checkoutForm.rentalDate);
      const today = new Date();
      today.setHours(0, 0, 0, 0);
    ...
    const rentalDurationNumber = Number(checkoutForm.rentalDurationDays);
    if (
      Number.isNaN(rentalDurationNumber) ||
      rentalDurationNumber < 1 ||
      rentalDurationNumber > 90
    ) {
      errors.push("Rental duration must be between 1 and 90 days");
    }
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <input
                      ...
                      required
                      disabled={isSubmitting}
                      value={checkoutForm.rentalDurationDays}
                      onChange={(e) =>
                        setCheckoutForm({
                          ...checkoutForm,
                    ...
                    {["delivery", "pickup"].map((method) => (
                      ...
                      >
                        <input
                          ...
                          value={method}
                          checked={checkoutForm.fulfillmentMethod === method}
                          onChange={(e) =>
                            setCheckoutForm({
                              ...checkoutForm,
