...
export default function BannersAdminPage() {
  ...
  const [form, setForm] = useState<Partial<Banner>>({ active: true, order: 0, position: "home" });
  ...
  const create = async () => {
    ...
    if (res.status === 201) { toast.success("Banner added"); setForm({ active: true, order: 0, position: "home" }); load(); }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="border p-4 rounded">
            ...
            <div className="space-y-3">
              ...
              <input placeholder="Position (e.g., home)" className="input input-bordered w-full" value={form.position || "home"} onChange={(e) => setForm({ ...form, position: e.target.value })} />
              ...
              {items.map((it) => (
                <li key={it.id} className="flex items-center justify-between border p-3 rounded">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500">position: {it.position}</div>
                  </div>
