...
const getAllProducts = asyncHandler(async (request, response) => {
  ...
  if(mode === "admin"){
      ...
      for (let i = 0; i < queryArray.length; i++) {
        ...
        if (queryParam.includes("filters")) {
          if (queryParam.includes("price")) {
            ...
            filterType = "category";
          } else if (queryParam.includes("manufacturer")) {
    ...
    // Cache product lists briefly to speed repeated navigations
...
const createProduct = asyncHandler(async (request, response) => {
  ...
  // Validate MongoDB ObjectId format for relations
  const isMongoId = (v) => typeof v === 'string' && /^[a-f\d]{24}$/i.test(v);
  if (!isMongoId(merchantId)) {
    throw new AppError("Invalid merchantId format", 400);
  }
  if (!isMongoId(categoryId)) {
    throw new AppError("Invalid categoryId format", 400);
  }
  if (brandId && !isMongoId(brandId)) {
    throw new AppError("Invalid brandId format", 400);
  }
  ...
  // Normalize optional fields to avoid Prisma validation errors
